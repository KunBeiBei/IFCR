...
class GCN(nn.Module):
    def __init__(self, 
        ...
        # fusion layer
        self.buy_conv5_user = GraphConv(emb_dim, emb_dim, activation=nn.LeakyReLU(), norm='right', bias=False, weight=False, allow_zero_in_degree=True)
        self.buy_conv5_item = GraphConv(emb_dim, emb_dim, activation=nn.LeakyReLU(), norm='right', bias=False, weight=False, allow_zero_in_degree=True)
        self.cart_conv5_user = GraphConv(emb_dim, emb_dim, activation=nn.LeakyReLU(), norm='right', bias=False, weight=False, allow_zero_in_degree=True)
        self.cart_conv5_item = GraphConv(emb_dim, emb_dim, activation=nn.LeakyReLU(), norm='right', bias=False, weight=False, allow_zero_in_degree=True)
        self.pv_conv5_user = GraphConv(emb_dim, emb_dim, activation=nn.LeakyReLU(), norm='right', bias=False, weight=False, allow_zero_in_degree=True)
        self.pv_conv5_item = GraphConv(emb_dim, emb_dim, activation=nn.LeakyReLU(), norm='right', bias=False, weight=False, allow_zero_in_degree=True)
...
if __name__ == '__main__':
    ...
    for epoch in tqdm.tqdm(range(traNum1, traNum2)):
        ...
        buy_temp = temp * torch.einsum('b,c->bc',  model.buy_prediction, model.buy_prediction)
        cart_temp = temp * torch.einsum('b,c->bc', model.cart_prediction, model.cart_prediction)
        pv_temp = temp * torch.einsum('b,c->bc', model.pv_prediction, model.pv_prediction)
    ...
    torch.save(recall10,  'jeg/T/M-Dyn/recall10.pth')
    torch.save(recall50,  'jeg/T/M-Dyn/recall50.pth')
    torch.save(recall100, 'jeg/T/M-Dyn/recall100.pth')
    torch.save(ndcg10,    'jeg/T/M-Dyn/ndcg10.pth')
    torch.save(ndcg50,    'jeg/T/M-Dyn/ndcg50.pth')
    torch.save(ndcg100,   'jeg/T/M-Dyn/ndcg100.pth')
    torch.save(losss,     'jeg/T/M-Dyn/loss.pth')
