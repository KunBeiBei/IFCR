...
class GCN(nn.Module):
    ...
    def forward(self, g):
        ...
        for i in range(self.l):
            buy_dst = self.buy_layer[f'item_conv_{i}'](g['buy'], (src_feature, dst_feature), weight=self.node_weights[f'weight_{i}'])
            buy_src = self.buy_layer[f'user_conv_{i}'](g['rev_buy'], (dst_feature, src_feature), weight=self.node_weights[f'weight_{i}'])

            cart_dst = self.cart_layer[f'item_conv_{i}'](g['cart'], (src_feature, dst_feature), weight=self.node_weights[f'weight_{i}'])
            cart_src = self.cart_layer[f'user_conv_{i}'](g['rev_cart'], (dst_feature, src_feature), weight=self.node_weights[f'weight_{i}'])

            pv_dst = self.pv_layer[f'item_conv_{i}'](g['pv'], (src_feature, dst_feature), weight=self.node_weights[f'weight_{i}'])
            pv_src = self.pv_layer[f'user_conv_{i}'](g['rev_pv'], (dst_feature, src_feature), weight=self.node_weights[f'weight_{i}'])
...
if __name__ == '__main__':
    ...
    torch.save(recall10,  'jeg/T/M-Beh/recall10.pth')
    torch.save(recall50,  'jeg/T/M-Beh/recall50.pth')
    torch.save(recall100, 'jeg/T/M-Beh/recall100.pth')
    torch.save(ndcg10,    'jeg/T/M-Beh/ndcg10.pth')
    torch.save(ndcg50,    'jeg/T/M-Beh/ndcg50.pth')
    torch.save(ndcg100,   'jeg/T/M-Beh/ndcg100.pth')
    torch.save(losss,     'jeg/T/M-Beh/loss.pth')
